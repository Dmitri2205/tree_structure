...
export const Aside = () => {
  ...
  const treeClickHandler = (e: any, child: object): void => {
    e.stopPropagation();
    const categoryParams: Array<string> =
      e.currentTarget.parentNode.id.split(":");
  ...
  const renderTreeStructure = (): Array<ReactNode> => {
    let structure: Array<ReactNode> = [];

    let categoryName = "";

    const renderChildren = (
      children: Array<any>,
      parentName: string | undefined | null = undefined,
      start: boolean
    ) => {
      if (start) {
    ...
    const renderProperties = (child: TChild , name: string) => {
      const { properties } = child;
      const propsArray: Array<object> = Object.entries(
            ...
            <Accordion.Body style={{ padding: "0" }}>
              <ListGroup style={{ textAlign: "left" }}>
                {propsArray.map((pair: Array<any>, i: number) => {
                  const pairHasChildren: boolean =
                    pair[1].hasOwnProperty("children");
                  return (
                    <ListGroup.Item
                      key={`params${i}ofChild${name}`}
